		...
		<td>
			<sf:form commandName="instanceNotifier" method="POST">
				<table id="id-form">
					...
					<tr>
						<th valign="top"><c:out value="Instance"/>:</th>
						<td>
							
						<sf:select path="instance.id">
							<c:forEach var="instance" items="${instances}">
								<sf:option value="${instance.id}">${instance.projectEnvironment.environment.name}-${instance.projectEnvironment.project.name} ${instance.name}</sf:option>
							</c:forEach>
						</sf:select>
						</td>
						<td>
							<c:set var="error"><sf:errors path="instance.id"/></c:set>
					...
					<jsp:include page="/WEB-INF/jsp/common/formElement.jsp">
						...
						<jsp:param name="items" value="postWorkerPlugins"/>
					</jsp:include>
					<jsp:include page="/WEB-INF/jsp/common/formElement.jsp">
